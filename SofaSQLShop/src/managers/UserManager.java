...
public class UserManager {
    ...
    public User addUser() {
        ...
        buyer.setCity(clientCity);
        
        System.out.print("Balance: "); 
        Double clientBalance = scanner.nextDouble();
        buyer.setBalance(clientBalance);
    ...
    public void printListUsers(List<User> users) {
        ...
        for (int i = 0; i < users.size(); i++) {
            double moneyBalance = users.get(i).getBuyer().getBalance();
            System.out.printf("%d. %s %s. Login: %s (balance: %s)%n",
                    users.get(i).getId(),
                    users.get(i).getBuyer().getName(),
                    users.get(i).getBuyer().getSurname(),
                    users.get(i).getLogin(),
                    //users.get(i).getClient().getPhone()
                    Math.round(moneyBalance * 100.0) / 100.0
    ...
    public void changeRole() {
        ...
        printListUsers(getDatabaseManager().getListUsers());
    ...
    public void editBuyer() {
        List <User> buyers = getDatabaseManager().getListUsers();
        System.out.println("----- Edit a Buyer -----");
        printListUsers(getDatabaseManager().getListUsers());

        System.out.print("Enter the number of the buyer to edit: ");
        int buyerIndex = Integer.parseInt(scanner.nextLine()) - 1;
    ...
    User user = buyers.get(buyerIndex);
    switch (task) {
                case 1:
                    System.out.print("New Firstname: ");
        String newFirstname = scanner.nextLine();
        if (!newFirstname.isEmpty()) {
            user.getBuyer().setName(newFirstname);
            break;
        }
        ...
        if (!newSurname.isEmpty()) {
            user.getBuyer().setSurname(newSurname);
        }
                    break;
                case 3:
        ...
        if (!newPhone.isEmpty()) {
            user.getBuyer().setPhone(newPhone);
        }
                    break;
                case 4:
        ...
        if (newBalance >= 0){
            user.getBuyer().setBalance(newBalance);
            break;
        }        
    ...
    public Buyer addBuyer() {
        ...
        String buyerSurname = scanner.nextLine();
    ...
    public User addMoney() {
        ...
        int buyerNumber = Integer.parseInt(scanner.nextLine());
        buyerNumber-=1;
        User user = users.get(buyerNumber);
